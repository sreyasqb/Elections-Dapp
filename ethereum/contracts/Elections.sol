...
contract Election{
    ...
    function getCandidateDetails(address c) public view returns (string memory,string memory,uint){
        return (candidates[c].name,candidates[c].party,candidates[c].voteCount);
    }
    ...
    function getResults() public view commissioner returns(address,uint){
        require(electionsStarted);
        uint maxCount=0;
        address winner;
        for (uint i=0;i<candidatesCount;i++){
            address cand=candidateAddresses[i];
            if (candidates[cand].voteCount>maxCount){
                winner=cand;
                maxCount=candidates[cand].voteCount;
            }
        }
